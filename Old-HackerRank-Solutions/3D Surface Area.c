...
for(int i=0; i<A_rows; i++){
        for (int j=0; j<A_columns; j++){
            ...
            if (h>0){
                total_area+=2;
                total_area+=(i==0)?h:(h > A[i-1][j] ? h-A[i-1][j]:0);
                total_area+=(i==A_rows-1)?h:(h > A[i+1][j] ? h-A[i+1][j]:0);
                total_area+=(j==0)?h:(h > A[i][j-1] ? h - A[i][j-1] : 0);
                total_area+=(j==A_columns-1) ? h:(h>A[i][j+1] ? h-A[i][j+1]:0);
            }
        }
    }
    return total_area;
